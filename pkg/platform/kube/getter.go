...
func (g *getter) get(consumer *consumer, getFunctionsOptions *platform.GetFunctionsOptions) ([]platform.Function, error) {
	...
	client, err := consumer.getNuclioClientSet(getFunctionsOptions.AuthConfig)
	...
	if getFunctionsOptions.Name != "" {
		...
		function, err := client.NuclioV1beta1().NuclioFunctions(getFunctionsOptions.Namespace).Get(getFunctionsOptions.Name, meta_v1.GetOptions{})
		if err != nil {
		...
		functionInstanceList, err := client.NuclioV1beta1().NuclioFunctions(getFunctionsOptions.Namespace).List(meta_v1.ListOptions{LabelSelector: getFunctionsOptions.Labels})
